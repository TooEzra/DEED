...
export default async function CaretakerMaintenancePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {data.items.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-slate-500">No maintenance requests</CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-4">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex gap-2">
                    <Badge variant={m.priority === "URGENT" || m.priority === "HIGH" ? "destructive" : "warning"}>
                      {m.priority}
                    </Badge>
                    <Badge variant={m.status === "RESOLVED" || m.status === "CLOSED" ? "success" : "info"}>
                      {m.status}
                    </Badge>
                  </div>
                </div>
